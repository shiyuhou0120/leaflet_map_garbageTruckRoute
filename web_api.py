...
def get_cafe_info_in_taipei():
    ...
    # ensure_ascii=False : 回傳內容為正常中文字，否則會是電腦才看得懂的程式碼
...
def get_cafe_info_in_hsinchu():
    ...
    # 自訂回應標頭，讓收到回應的 User Agent (例如瀏覽器、手機 APP 等) 可以知道回應相關的訊息與設定
    response.headers["Content-Type"] = "application/json"
    response.headers['Access-Control-Allow-Origin'] = '*' # 對所有人公開，沒有跨域問題
    return response
...
if __name__ == '__main__':
    app.run(
        ...
        port=5000 
    ) # http://localhost:5000/cafe_taipei    localhost表示本機IP